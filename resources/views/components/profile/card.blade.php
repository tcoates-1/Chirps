<div {{$attributes->merge(['class'=>'grid grid-cols-2 gap-4 border rounded-lg p-2']) }}>
    ...
    <div class="flex">
        ...
        <div>
            <h1 class="card-header align-middle md:text-3xl text-blue-600 leading-tight hover:font-bold">
                <a href="{{ route('profile.show', ['username' => $user->username]) }}">
                    {{ $user->username }}
                </a>
            </h1>
    ...
    <div class="shrink justify-end col-start-2">
        <h1 class="card-header text-right md:text-3xl text-blue-600 leading-tight hover:font-bold">
            <a href="{{ route('profile.show', ['username' => $user->username]) }}">
                {{ $user->name }}
            </a>
        </h1>
